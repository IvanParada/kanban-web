...
import { ApiResponse, ConfirmPayload, CreateTaskPayload, GetTasksResponseDto, PresignPayload, PresignResponse, Task, TaskImage } from '../interfaces/task.models';
...
export class TasksService {
    ...
    createTask(task: CreateTaskPayload): Observable<Task> {
        return this.http.post<ApiResponse<Task>>(`${this.baseUrl}/tasks`, task).pipe(
            map(resp => resp.data)
        );
    }
    ...
    refreshSignedUrl(imageId: string): Observable<{ signedUrl: string }> {
        return this.http.get<{ signedUrl: string }>(`${this.baseUrl}/task-images/${imageId}/signed-url`);
    }
    ...
    presignTaskImages(payload: PresignPayload): Observable<PresignResponse[]> {
        return this.http.post<ApiResponse<PresignResponse[]>>(`${this.baseUrl}/task-images/presign`, payload).pipe(
            map(resp => resp.data)
        );
    }

    confirmTaskImages(payload: ConfirmPayload): Observable<TaskImage[]> {
        return this.http.post<ApiResponse<TaskImage[]>>(`${this.baseUrl}/task-images/confirm`, payload).pipe(
            map(resp => resp.data)
        );
    }
